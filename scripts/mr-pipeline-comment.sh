...
PROJECT_PATH="${CI_PROJECT_PATH:-HttpAnimations/er}"
MARKER="<!-- mr-pipeline-${PIPELINE_ID} -->"
PIPELINE_URL="${SERVER_URL}/${PROJECT_PATH}/-/pipelines/${PIPELINE_ID}"
RUN_URL="${RUN_ID:+https://github.com/justacalico/er/actions/runs/${RUN_ID}}"
...
GitHub run: [View on GitHub](${RUN_URL})"
  ...
  finish)
    ...
    if [ -n "$RUN_ID" ] && command -v gh >/dev/null 2>&1; then
      jobs=$(gh run view "$RUN_ID" -R justacalico/er --json jobs 2>/dev/null | jq -r '.jobs[] | select(.conclusion != null or .status != null) | "- **\(.name)**: \(.conclusion // .status)"' || true)
      downloads=$(gh api "repos/justacalico/er/actions/runs/${RUN_ID}/artifacts?per_page=100" 2>/dev/null \
        | jq -r --arg run "$RUN_ID" '.artifacts[] | select(.expired != true) | "- [\(.name)](https://github.com/justacalico/er/actions/runs/\($run)/artifacts/\(.id))"' || true)
    fi

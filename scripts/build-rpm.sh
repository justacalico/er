...
# Installs to /opt/er with a /usr/bin/er symlink, matching the
...
cat > "$TOPDIR/SPECS/er.spec" <<'EOF'
Name: er
Version: %{pkg_version}
Release: %{pkg_release}%{?dist}
Summary: er desktop client
License: AGPL-3.0-only
URL: https://gitlab.com/HttpAnimations/er
...
%global __provides_exclude_from ^/opt/er/.*

%description
Run the same flatpak multiple times in isolated instances.

%install
mkdir -p %{buildroot}/opt/er %{buildroot}/usr/bin
